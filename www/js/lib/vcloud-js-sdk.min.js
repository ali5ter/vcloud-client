...
"application/vnd.vmware.vcloud.instantiateVAppTemplateParams+xml");vmware.rest.post(e[g]+"/action/instantiateVAppTemplate",i).done(function(d){a.once(vmware.events.cloud.TASK_START,a.updateModels);a.taskManager.newTask($.parseXML(d.xml?d.xml:(new XMLSerializer).serializeToString(d)))})});a.fleshOutTemplate(templateObj);return!0};a.searchCatalogFaceted=function(d){y?E(d)():(a.once(vmware.events.cloud.TEMPLATE_REFRESH,E(d)),a.getAllTemplates())};a.searchCatalog=function(d){a.searchCatalogFaceted({searchTerm:d})};
a.taskHistory=function(){return a.taskManager.taskLog()};a.updateModels=function(){a.fetchURL(b.concat("query?type=vm&format=records&pageSize=1024"),"GET",null,F)};a.begin=function(){a.metadata=new Metadata(b);a.taskManager=new TaskManager(a);a.register(vmware.events.cloud.TASK_COMPLETE,a.updateSingle);D("orgVdcNetwork","OrgVdcNetworkRecord",n);D("orgVdc","OrgVdcRecord",e);a.taskManager.autoRefresh=function(d){a.fetchURL(b.concat("query?type=task&filter=(status==running)"),"GET",null,d)};a.fetchURL(b.concat("query?type=task&pageSize=15&sortDesc=startDate"),
"GET",null,a.taskManager.loadTasksLog);a.updateModels();a.getAllTemplates();a.once(vmware.events.cloud.TEMPLATE_REFRESH,function(){a.metadata.register(a.metadata.filterTemplates("downloads"),function(d){for(t in d)a.getTemplateFromHref(t).setAttr("downloads",d[t])})})};a.updateSingle=function(d){d=d.eventData.data;r=!0;a.fetchURL(d.attr.ownerHref,"GET",null,A(a.getVAppFromHref(d.attr.ownerHref)||new vApp(a),!0))};a.updateCatalog=function(d,e){for(var g=(d-1)*e;g<d*e;g++)if(null==h[g])return a.fetchURL(b.concat("query?type=vAppTemplate&format=records&page="+
d+"&pageSize="+e),"GET",null,x(d,e,!1));a.trigger(vmware.events.cloud.TEMPLATE_REFRESH,h)};a.getCatalog=function(){return h};a.getVMStatus=function(d){for(var e=a.taskManager.inProgress(),b=0;b<e.length;b++)if(null!=d.getID().match(e[b]))return d.STATUS_WORKING;return d.getStatus()};a.percentageDoneUpdate=function(){return 0!=q?100*s/q:0};a.getVApps=function(d){list=[];for(var e in i.vapps)list.push(i.vapps[e]);if(null!=d){switch(d){case a.SORTBY.NAME:sorter=function(a,d){return a.getName()>d.getName()};
break;case a.SORTBY.DATE:sorter=function(a,d){return a.getCreationDate()>d.getCreationDate()}}return list.sort(sorter)}return list};a.SORTBY={NAME:0,DATE:1};a.getVMs=function(){list=[];for(var a in i.vms)list.push(i.vms[a]);return list};a.getVM=function(a){return i.vms[a]};a.getVApp=function(a){return i.vapps[a]};a.getVObject=function(d){return a.getVApp(d)||a.getVM(d)};a.getVAppFromHref=function(a){for(var e in i.vapps)if(i.vapps[e].getAttr("href")==a)return i.vapps[e]};a.getTemplateFromHref=function(a){for(var e in h)if(h[e].getAttr("href")==
a)return h[e]};a.powerOff=function(a){u(a,"power:powerOff")};a.powerOn=function(a){u(a,"power:powerOn")};a.suspend=function(a){u(a,"power:suspend")};a.deleteVApp=function(a){u(a,"remove")};a.displayConsole=function(d,e){a.once(vmware.events.cloud.NEW_TICKET,function(){});e.getConsoleTicket()};a.saveCache=function(){for(var d={vapps:[]},e=a.getVApps(),b=0;b<e.length;b++)d.vapps.push(e[b].save());d.vms=[];e=a.getVMs();for(b=0;b<e.length;b++)d.vms.push(e[b].save());d.catalog=[];e=a.getCatalog();for(b=
0;b<e.length;b++)d.catalog.push(e[b].save());d.tasks=a.taskManager.saveLog();return JSON.stringify(d)};a.loadCache=function(d){d=JSON.parse(d);i={vapps:{},vms:{}};h=[];for(var e,b=0;b<d.vapps.length;b++)e=new vApp(a),e.load(d.vapps[b]),i.vapps[e.getID()]=e;for(b=0;b<d.vms.length;b++)e=new vM(a),e.load(d.vms[b]),i.vms[e.getID()]=e;for(b=0;b<d.catalog.length;b++)e=new Template,e.load(d.catalog[b]),h.push(e);a.taskManager.loadLog(d.tasks);a.trigger(vmware.events.cloud.REFRESH_COMPLETE)};return a};vmware="undefined"==typeof vmware||!vmware?{}:vmware;vmware.events.log=[];
...
"</soapenv:Body></soapenv:Envelope>",headers:b,dataType:"xml",cache:!1})}}}();function TaskManager(b){var f,g={},i={},j=function(){this.attr={}};j.prototype.addAttr=function(b,a){this.attr[b]=a};var h={autoRefresh:null,update:function(){clearTimeout(f);var e=!1;for(key in g)b.fetchURL(g[key].attr.href,"GET",null,h.refreshTask),e=!0;h.autoRefresh&&h.autoRefresh(h.loadTasks);f=e?setTimeout("cloud.taskManager.update();",5E3):setTimeout("cloud.taskManager.update();",1E4)},loadTasks:function(b){h.loadTasksTo(g)(b)},loadTasksLog:function(b){h.loadTasksTo(i)(b)},loadTasksTo:function(b){return function(a){var g,
